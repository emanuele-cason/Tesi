...
\noindent
Si osserva qualitativamente come le due misure siano differenti per un offset pressocché costante. Tale differenza è imputabile a un elevato numero di fattori, come la geometria del tubo di Pitot, l'errore sistematico di misura del sensore, gli errori di postazione (ovvero di posizionamento del tubo rispetto al flusso indisturbato) e molti altri, che in generale sono di difficile previsione tramite approccio analitico. Al fine di disporre di dati utilizzabili quindi, va operata una calibrazione di massima del sensore.
...
\noindent
Una volta selezionato tale intervallo all'interno del software appositamente creato, quest'ultimo trasla il set di dati di velocità dell'aria verso l'alto di una quantità costante, imponendo che la velocità media coincida con la media della velocità rispetto al suolo (misurata dal modulo GNSS), nello stesso intervallo.
\\\\
