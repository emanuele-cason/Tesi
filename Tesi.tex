...
\item[2.2] Nota la potenza disponibile ($P_A$) e i dati di progetto scelti allo step 1 ($M_{TOT}$, $X_{TO}$ e $C_{L, max}$), viene calcolata la velocità di decollo. In questo caso le ipotesi introdotte, da verificare, sono quelle di drag e attrito volvente trascurabili ($D = 0$, $F_{AV} = 0$). 
...
Tra i dati raccolti dal sistema di telemetria, ci sono quelli di velocità rispetto al suolo, ricavati dal GPS e di accelerazione sui tre assi, provenienti dagli accelerometri. A partire da questi dati, sempre sulla base del secondo principio della dinamica, è possibile ricavare la forza risultante agente sul velivolo, pari alla somma vettoriale delle forze di propulsione ($\overrightarrow{T}$), resistenza aerodinamica ($\overrightarrow{D}$) e attrito volvente ($\overrightarrow{F_{AV}}$). L'obbiettivo di tale processo è il ricavare la forza risultante orizzontale, realmente agente sul velivolo in fase di decollo, per confrontarla con la forza propulsiva.
\\\\
Per fare ciò, è stato sviluppato uno script specifico in codice Python. Tale programma per funzionare richiede in input gli istanti di inizio del decollo e di liftoff. Per determinarli permette all'utente la loro selezione, direttamente sul diagramma di velocità e altitudine in funzione del tempo, rispettivamente attraverso un marker verticale verde e uno rosso. Successivamente, una seconda finestra mostra gli istanti selezionati sul grafico delle accelerazioni, consentendo la verifica della correttezza dei tempi scelti.
